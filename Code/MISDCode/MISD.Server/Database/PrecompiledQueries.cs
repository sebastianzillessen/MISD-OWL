...
namespace MISD.Server.Database
{
    ...
    public static class PrecompiledQueries
    {
        ...
        public static Func<MISDDataContext, int, List<PluginMetadata>> GetPluginsByMonitoredSystemID = CompiledQuery.Compile<MISDDataContext, int, List<PluginMetadata>>((dataContext, monitoredSystemID) => dataContext.Indicator.Where(q => q.MonitoredSystemID == monitoredSystemID).Select(q => q.PluginMetadata).GroupBy(q => q.ID, (key, p) => new{plugin = p}).Select(p => p.plugin.First()).ToList());
        ...
        public static Func<MISDDataContext, int, long, MappingState> GetMonitoredSystemStateByID =
            CompiledQuery.Compile<MISDDataContext, int, long, MappingState>
                ((dataContext, monitoredSystemID, workstationReset) =>
                (MappingState)dataContext.Indicator.Where
                (p => p.MonitoredSystemID == monitoredSystemID).Select
                (p => new { p.ID, p.MappingDuration }).Join(dataContext.IndicatorValue.Where
                (z => z.Mapping > 0).Select
                (p => new { p.IndicatorID, p.Timestamp, p.Mapping }), p => p.ID, q => q.IndicatorID, (p, q) => new { q.Timestamp, p.MappingDuration, q.Mapping }).Where
                (p => p.Timestamp > Math.Max(DateTime.Now.Ticks - workstationReset, DateTime.Now.Ticks - p.MappingDuration)).Select
                (p => p.Mapping).Max());
