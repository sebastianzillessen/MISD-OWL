...
{
    /// <summary>
    /// Sheduling class for the server-sided global data acquisition.
    /// </summary>
    class GlobalJobScheduler
    {
        #region Singleton

        private static volatile GlobalJobScheduler instance;
        private static object syncRoot = new Object();

        /// <summary>
        /// Property to access the class.
        /// </summary>
        public static GlobalJobScheduler Instance
        {
            get
            {
                if (instance == null)
                {
                    ...
                    {
                        if (instance == null)
                            instance = new GlobalJobScheduler();
                    }
                }
        ...
        private List<Tuple<int, WorkstationInfo, IPlugin, string, int>> jobs;
        ...
        {
            jobs = new List<Tuple<int, WorkstationInfo, IPlugin, string, int>>();
        ...
        private void timer_Elapsed(object sender, ElapsedEventArgs e)
        {
            ...
            WaitCallback callback = new WaitCallback(UpdateValues);

            // check which jobs need to run
            foreach (var job in jobs)
            {
                if ((tickNumber - job.Item5) > 0 && (tickNumber + 1 - job.Item5) % job.Item1 == 1)
                {
        ...
        {
            Tuple<int, WorkstationInfo, IPlugin, string, int> currentJob = (Tuple<int, WorkstationInfo, IPlugin, string, int>)job;
            UpdateValues(currentJob.Item2, currentJob.Item3, currentJob.Item4);
        }

        /// <summary>
        /// The method that aquires, filters and sends the indicator values.
        /// </summary>
        public void UpdateValues(WorkstationInfo node, IPlugin plugin, string indicator)
        {
            try
            {
                if (node.IsAvailable)
                {
                    // Aquire data
                    var values = GetValues(node, plugin, indicator);
            ...
            {
                MISD.Core.Logger.Instance.WriteEntry("GlobalJobScheduler_UpdateValues: Problem with node " + node.Name + " and plugin " + plugin.GetName() + " and indicator " + indicator + ": " + e.ToString(), LogType.Exception);
            }
        }
        ...
        {
            var temp = plugin.AcquireData(new List<String> { indicator }, node.FQDN);
            if (temp == null)
            {
        ...
        public void UpdateJob(int newInterval, int nodeID, string pluginName, string indicatorName)
        {
            if (newInterval >= 1)
            {
                Tuple<int, WorkstationInfo, IPlugin, string, int> oldJob = null;
                foreach (var job in jobs)
                {
                ...
                if (oldJob != null)
                {
                    AddJob(newInterval, oldJob.Item2, oldJob.Item3, oldJob.Item4);
        ...
        {
            List<Tuple<int, WorkstationInfo, IPlugin, string, int>> toBeRemoved = new List<Tuple<int, WorkstationInfo, IPlugin, string, int>>();

            foreach (var job in jobs)
            {
                if (job.Item2.Equals(node) && job.Item3.Equals(plugin) && job.Item4.Equals(indicator))
                {
                    toBeRemoved.Add(job);
                }
            }
