...
{
    public class MainRefreshTimerJob : TimerJobBase
    {
        #region Constructor

        public MainRefreshTimerJob(TimeSpan interval)
        {
            this.ID = "MainRefreshTimerJob";
        ...
        {
            //refresh schedulers
            MainScheduler.Instance.RefreshJobs();

            //refresh the MainRefreshTimerJob
            var jobs = new Dictionary<string, TimeSpan>();
            jobs.Add("MainRefreshTimerJob", Properties.Settings.Default.MainUpdateInterval);
            MainScheduler.Instance.RefreshUpdateInterval(jobs);
        }
